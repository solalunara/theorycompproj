...
\section{Methodology}
    ...
    it may be more computationally expensive, the Runge-Kutta method is a good choice for this project, specifically
    RK4\cite{Strogatz2000}. As such, RK4 was used to calculate the steps of angular velocity using the equation of
    motion for the forced, damped pendulum, and Euler's method was used to calculate the angle from the angular velocity, 
    as since the angular velocities are calculated discretely with RK4 there is no way to take a half step which is required 
    for another iteration RK4 in a way that would provide a benefit to the numerical computation. The equation of motion
...
\section{Results}
    ...
    the selection of driving forces for the Poincar\'e sections.\\
    \subsection{Poincar\'e Plots}
    \subsection{Lyapunov Exponent}
    \subsection{Phase Space Analysis}
\section{Conclusions}
